> **注意：本文件是改造前的样式摘录，已被设计令牌层取代。**
>
> 现全站样式以 `src/styles/_tokens.scss`（设计令牌）、`src/styles/_base.scss`（全局基础与
> 工具类）、`src/styles/_element.scss`（Element Plus 主题覆盖）为唯一来源。
> 本文件中的硬编码色值（`#4A9C8C`、`#fb923c`、`#7ed321`、`#2d3436` 等）、
> 固定宽度（`1200px` / `980px`）与 emoji 图标用法**均已废弃**，请勿再复制到新代码里。
> 保留本文件仅为对照历史实现。
