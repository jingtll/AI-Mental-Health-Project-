...
**TS status (core layer):** `config` / `utils/request` / `api` / `router` / `stores` / `main` are TypeScript. All `.vue` business pages still use plain `<script setup>` (JS). Do not assume `lang="ts"` unless you see it.
