...
| `src/utils/request.ts` | Shared axios (default export `http`); all normal HTTP goes through this |
| `src/utils/session.ts` | Intended single reader/writer of `token` + `userInfo` (avoids the old `userinfo` key typo); router/request still touch `localStorage` in places |
...
- HTTP helper: `src/utils/request.ts` **default-exports** typed `http` (`import http from "@/utils/request"`). `get/post/put/delete` → `Promise<T>`; interceptors already unwrap `data.data`. Do not use raw axios for app APIs.
...
- Intentional path spellings — do not “fix” without updating all imports: `BackenLayout.vue`, `emotionDairy.vue` (diary).
...
- README claims GitHub Actions CI (`.github/workflows/ci.yml`); this working tree has **no** `.github/workflows`. Trust the repo scripts (`typecheck` / `build`), not that CI section.
- Router guard in `src/router/index.ts` still reads `localStorage` directly (does not go through `session.ts`). Prefer `session.ts` for new session reads/writes; leave the guard alone unless fixing auth bugs.
